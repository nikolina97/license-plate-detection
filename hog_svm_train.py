...
def process_image(image, step_size, window_size, hog, clf_svm):
    best_score = 0
    best_window = None
    for y in range (0, image.shape[0], step_size):
        for x in range (0, image.shape[1], step_size):
            ...
            if window.shape == (window_size[1], window_size[0]):
                score = classify_window(window, hog, clf_svm)
                if score > best_score:
                    best_score = score
                    best_window = window
                    score_window = this_window
    return best_score, best_window, score_window
